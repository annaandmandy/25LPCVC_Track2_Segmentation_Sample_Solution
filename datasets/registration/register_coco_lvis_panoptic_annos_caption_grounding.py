...
_PREDEFINED_SPLITS_COCO_PANOPTIC_CAPTION = {
    "coco_2017_train_panoptic_filtrefgumdval": (
        ...
        "lvis/coco_train2017_filtrefgumdval_lvis.json",
    ),
...
def load_coco_panoptic_json(json_file, image_dir, gt_dir, semseg_dir, caption_file, grounding_file, lvis_file, meta):
    """
    Args:
    ...
    with PathManager.open(lvis_file) as f:
        lvis_info = json.load(f)
    ...
    # build dictionary for lvis
    lvis_dict = collections.defaultdict(list)
    for lvis_ann in lvis_info['annotations']:
        image_id = int(lvis_ann["image_id"])
        lvis_dict[image_id].append(lvis_ann)
    ...
    for ann in json_info["annotations"]:
        ...
        lvis_anno = lvis_dict[image_id] if image_id in lvis_dict else []
        ret.append(
            {
...
def register_coco_panoptic_annos_caption_grounding_sem_seg(
    name, metadata, image_root, panoptic_root, panoptic_json, sem_seg_root, caption_root, grounding_root, similarity_pth, lvis_root, instances_json,
):
    ...
    semantic_name = name + "_with_sem_seg_caption_grounding_lvis"
    DatasetCatalog.register(
        semantic_name,
        lambda: load_coco_panoptic_json(panoptic_json, image_root, panoptic_root, sem_seg_root, caption_root, grounding_root, lvis_root, metadata),
    )
...
def register_all_coco_panoptic_annos_caption_grounding_sem_seg(root):
    for (
        prefix,
        (panoptic_root, panoptic_json, semantic_root, caption_root, grounding_root, similarity_pth, lvis_root),
