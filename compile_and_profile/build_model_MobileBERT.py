...
def build_model_MobileBERT(image_input, text_input, output_path="./compile_and_profile", test_torch_model_local=True):
    """
    ...
    # Replace the language encoder with MobileBERT
    tokenizer = MobileBertTokenizer.from_pretrained("google/mobilebert-uncased")
    lang_encoder = MobileBertModel.from_pretrained("google/mobilebert-uncased").to(device)
    lang_encoder.load_state_dict({k.replace('sem_seg_head.predictor.lang_encoder.', ''): v for k,v in ckpt.items() if 'sem_seg_head.predictor.lang_encoder' in k}, strict=True)


    with torch.no_grad():
        lang_encoder.eval()
        inputs = tokenizer(["background"], return_tensors="pt", padding=True, truncation=True).to(device)
        embeds = lang_encoder(**inputs).last_hidden_state.mean(dim=1)  # Average pooling for embeddings
        lang_encoder.default_text_embeddings = embeds
